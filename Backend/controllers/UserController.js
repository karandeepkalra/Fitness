...
const paymRazorpay = async (req, res) => {
  try {
    ...
    if (!appointmentData.amount) {
      console.log('No amount found in appointment data');
      return res.status(400).json({ 
        success: false, 
...
const verifyRazorpay = async (req, res) => {
  try {
    ...
    const { razorpay_order_id, razorpay_payment_id, appointmentId } = req.body;

    // Validate required fields
    if (!razorpay_order_id || !razorpay_payment_id || !appointmentId) {
      ...
      return res.status(400).json({
        success: false,
        message: "Missing required fields: razorpay_order_id, razorpay_payment_id, or appointmentId"
      });
    }
    ...
    console.log('Valid request with:', { 
      razorpay_order_id, 
      razorpay_payment_id,
      appointmentId,
      updatedAppointment 
